...
export const generateRandomData = (length:number,companyArray?:string[]) => {
  ...
  function generateRandomCompanyData() {
    ...
    const lastTenYearData = Array.from({ length: 10 }, (_, index) => ({
      ...
      totalRevenueValue: Math.floor(Math.random() * (100000 - 10000) + 10000),
      profitValue: Math.floor(Math.random() * (10000 - 1000) + 1000),
      employeeCount:Math.floor(Math.random() * (1000 - 100) + 10),
    }));
  ...
  const companies = Array.from({ length: 5 }, (_, index) => ({
    name: companyArray ? companyArray[index] :generateSlug(2, { format: "title" }),
